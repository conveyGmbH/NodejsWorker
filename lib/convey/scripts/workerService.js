                ...
                this._promise.then(function () {
                    if (that.status === WorkerService.statusId.started) {
                        ...
                        try {
                            ...
                            Log.print(Log.l.error, "exception occured in activity! " + (exception && exception.message));
                        }
                    ...
                    Log.print(Log.l.trace, "onComplete status=" + that.status + " nextStatus=" + that._nextStatus);
                    if (that._nextStatus) {
                    ...
                    Log.print(Log.l.error, "onError status=" + that.status + " nextStatus=" + that._nextStatus);
                    if (that._nextStatus) {
                        Log.print(Log.l.info, "now switch to status=" + that._nextStatus);
                        that._status = that._nextStatus;
                        that._nextStatus = null;
        ...
        */
        WorkLoop: WinJS.Class.define(function workLoop(dispatcherNames, port) {
            ...
            _requestHandler: function (req, res) {
                ...
                if (process && process.env && process.env.WEBSITE_SITE_NAME) {
                    bodyText += process.env.WEBSITE_SITE_NAME + "\n";
                }

                bodyText += "\nService settings:" + 
                    "\n hostName=" + AppData.appSettings.odata.hostName +
                    "\n onlinePath=" + AppData.appSettings.odata.onlinePath + 
                    "\n";
                ...
                for (i = 0; i < this.dispatcherCount; i++) {
                    ...
                    if (curDispatcher.lastCall) {
						bodyText += "\nlast call: " + curDispatcher.lastCall.toISOString();
                    }
            ...
            */
            start: function () {
                ...
                if (this._server && this._port && !this._listening) {
                    ...
                    Log.print(Log.l.trace,"start listen!");
