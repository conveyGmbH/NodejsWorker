...
(function() {
    ...
    WinJS.Utilities._require([
        'WinJS/Core/_Global'
    ], function loggingInit(_Global) {
        platform = _Global.navigator && _Global.navigator.platform || _Global.platform || "node";
    });
    var isWindows = (typeof platform === "string" ? platform.toLowerCase() : "") === "win32";
    var eol = isWindows ? "\r\n" : "\n";
                                            ...
                                            dirEntry.getFile(fileName, {
                                                ...
                                                var deleteFile = function () {
                                                    fileEntry.remove(function () {
                                                        console.log("file deleted!" + eol);
                                                    },
                                                        function (errorResponse) {
                                                            console.log("file delete: Failed remove file " + fileName + " error: " + JSON.stringify(errorResponse) + eol);
                                                        },
                                                        function () {
                                                            console.log("file delete: extra ignored!" + eol);
                                                        });
                                                }
                                                deleteFile();
                                            }, function (errorResponse) {
                                                console.error("getFile(" + fileName + ") error: " + JSON.stringify(errorResponse) + eol);
                                                that._inDataFile = false;
                                            });
                                        } catch (ex) {
                                            console.error("Failed dirEntry.getFile error: " + JSON.stringify(ex) + eol);
                                            that._inDataFile = false;
                                        }
                                    } else {
                                        console.error("Delete file error NO dirEntry!" + eol);
                                        that._inDataFile = false;
                                    }
                                }, function (errorResponse) {
                                    console.error("resolveLocalFileSystemURL error: " + JSON.stringify(errorResponse) + eol);
                                    that._inDataFile = false;
                                });
                            } catch (ex) {
                                console.error("Failed resolveLocalFileSystemURL error: " + JSON.stringify(ex) + eol);
                                that._inDataFile = false;
                            }
                        } else {
                            console.error("resolveLocalFileSystemURL is undefined" + eol);
                            that._inDataFile = false;
                        }
                    }
                },
                            ...
                            try {
                                window.resolveLocalFileSystemURL(dataDirectory, function (dirEntry) {
                                    if (dirEntry) {
                                                        ...
                                                        fileEntry.createWriter(function (fileWriter) {
                                                            if (fileWriter) {
                                                                fileWriter.onerror = function (errorResponse) {
                                                                    console.error("Failed write file " + fileName + " error: " + JSON.stringify(errorResponse) + eol);
                                                                    that._inDataFile = false;
                                                                };
                                                                fileWriter.onwriteend = function () {
                                                                    //console.log( "Successful file read! fileExt=" + fileExt + " data-length=" + data.length + eol);
                                                                    that._inDataFile = false;
                                                                };
                                                                try {
                                                                    fileWriter.seek(fileWriter.length);
                                                                } catch (ex) {
                                                                    console.error("Failed seek to end at " + fileWriter.length + " of file " + fileName + " error: " + JSON.stringify(ex) + eol);
                                                                }
                                                                try {
                                                                    fileWriter.write(data);
                                                                } catch (ex) {
                                                                    console.error("Failed write file " + fileName + " error: " + JSON.stringify(ex) + eol);
                                                                    that._inDataFile = false;
                                                                }
                                                            } else {
                                                                console.error("file write error NO fileWriter!" + eol);
                                                                that._inDataFile = false;
                                                            }
                                                        }, function (errorResponse) {
                                                            console.error("file write error: " + JSON.stringify(errorResponse) + eol);
                                                            that._inDataFile = false;
                                                        });
                                                    } catch (ex) {
                                                        console.error("Failed fileEntry.createWriter error: " + JSON.stringify(ex) + eol);
                                                        that._inDataFile = false;
                                                    }
                                                } else {
                                                    console.error("file write error NO fileEntry!" + eol);
                                                    that._inDataFile = false;
                                                }
                                            }, function (errorResponse) {
                                                console.error("getFile(" + fileName + ") error: " + JSON.stringify(errorResponse) + eol);
                                        ...
                                        console.error("file write error NO dirEntry!" + eol);
                                        that._inDataFile = false;
                                    }
                                }, function (errorResponse) {
                                    console.error("resolveLocalFileSystemURL error: " + JSON.stringify(errorResponse) + eol);
                    ...
                    if (!this._inDataFile && this._lineBuffer && this._lineBuffer.length > 0) {
                        ...
                        for (var i = 0; i < writeLineBuffer.length; i++) {
                            if (writeLineBuffer[i]) {
                                writeBuffer += writeLineBuffer[i] + eol;
                            }
                        }
                        if (writeBuffer) {
