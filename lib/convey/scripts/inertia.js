    ...
    WinJS.Namespace.define("TouchPhysics", {
        ...
        ManipulationEvents: WinJS.Class.define(
            function manipulationEvents(manipulationStarted, manipulationDelta, manipulationCompleted) {
                this._manipulationCompleted = manipulationStarted;
        ...
        ManipulationProcessor: WinJS.Class.define(
            function manipulationProcessor() {
                ...
                _supportedManipulations: MANIPULATION_TRANSLATE_X | MANIPULATION_TRANSLATE_Y,
        ...
        InertiaProcessor: WinJS.Class.define(
            function inertiaProcessor() {
                ...
                processTime: function (timeStamp) {
                    if (!this._completed) {
                        ...
                        if (this._desiredDeceleration > 0) {
                            var dt = timeStamp - this._initialTimeStamp;
                            var vD = this._desiredDeceleration * dt;
                            var decl = this._desiredDeceleration * dt * dt;
                            if (this._initialVelocityX > vD) {
                                sX = this._initialVelocityX * dt - decl;
                            } else {
                                var tMaxX = this._initialVelocityX / this._desiredDeceleration;
                                sX = this._initialVelocityX * tMaxX - this._desiredDeceleration * tMaxX * tMaxX;
                            }
                            if (this._initialVelocityY > vD) {
                                sY = this._initialVelocityY * dt - decl;
                            } else {
                                var tMaxY = this._initialVelocityY / this._desiredDeceleration;
                                sY = this._initialVelocityY * tMaxY - this._desiredDeceleration * tMaxY * tMaxY;
                            }
                        }
                ...
                reset: function () {
                    ...
                    this.x = this._initialOriginX;
                    this.y = this._initialOriginY;
                    this._completed = false;
                },
                _boundaryBottom: 0,
                boundaryBottom: {
            ...
            function manipulationEventSink(onTouch, inert, manip) {
                ...
                TouchPhysics.ManipulationEvents.apply(this, [
                    function manipulationStarted(x, y) {
                        ...
                        var pxX = TOUCH_COORD_TO_PIXEL(x);
                        var pxY = TOUCH_COORD_TO_PIXEL(x);
                        ...
                        if (inert) {
                            ...
                            inert.desiredDeceleration = TouchPhysics.touchIntertiaDeceleraion / 1000;
                        ...
                        if (!that.fCancelled) {
                            ...
                            var pxX = TOUCH_COORD_TO_PIXEL(that.fX);
                            var pxY = TOUCH_COORD_TO_PIXEL(that.fY);
        ...
        TouchPhysics: WinJS.Class.define(
            function touchPhysics(touchMove) {
                this.manipProc = new ManipulationProcessor();
                this.inertProc = new InertiaProcessor();
                this.manipEventSink = new ManipulationEventSink(touchMove, this.inertProc, this.manipProc);
                this.inertEventSink = new ManipulationEventSink(touchMove, this.inertProc);
                this.inertEventSink.setTimer(this.onTimer);
            }, {
                manipProc: null,
                inertProc: null,
                inertEventSink: null,
